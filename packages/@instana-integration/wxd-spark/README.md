# @instana-integration/wxd-spark

The Instana integration package is designed to showcase observability capabilities within Instana by monitoring the Watsonx.data Spark Engine. Spark exposes JMX metrics through OpenTelemetry, which can be effectively leveraged to track real-time metrics using the provided dashboards.
...
```shell
$ stanctl-integration download --package @instana-integration/wxd-spark
```

Importing the package into Instana:

```shell
$ stanctl-integration import --package @instana-integration/wxd-spark\
