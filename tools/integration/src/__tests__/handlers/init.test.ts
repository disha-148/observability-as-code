...
describe('Init Handler', () => {
    ...
    describe('handleInit', () => {
        ...
        it('should create selected config type folders', async () => {
            ...
            (checkbox as jest.Mock).mockResolvedValue(['dashboards', 'events', 'entities', 'infra-smart-alerts']);
            (utils.generateReadme as jest.Mock).mockImplementation(() => {});
            (utils.printDirectoryTree as jest.Mock).mockImplementation(() => {});
            ...
            expect(fs.mkdirSync).toHaveBeenCalledWith('/test/dir/test-package/infra-smart-alerts', { recursive: true });
        });
